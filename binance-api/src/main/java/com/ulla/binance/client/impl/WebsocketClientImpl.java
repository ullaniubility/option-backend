package com.ulla.binance.client.impl;
...
import com.ulla.binance.client.WebsocketClient;
import com.ulla.binance.client.enums.DefaultUrls;
import com.ulla.binance.client.exceptions.BinanceConnectorException;
import com.ulla.binance.client.utils.*;

import okhttp3.Request;

/**
 * <h2>Websocket Streams</h2> All stream endpoints under the
 * <a href="https://binance-docs.github.io/apidocs/spot/en/#websocket-market-streams">Websocket Market Streams</a> and
 * <a href="https://binance-docs.github.io/apidocs/spot/en/#user-data-streams">User Data Streams</a> section of the API
 * documentation will be implemented in this class. <br>
...
public class WebsocketClientImpl implements WebsocketClient {
    ...
    /**
     * Same as {@link #aggTradeStream(String, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param symbol
    ...
    public int aggTradeStream(String symbol, WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
        ...
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        Request request =
            RequestBuilder.buildWebsocketRequest(String.format("%s/ws/%s@aggTrade", baseUrl, symbol.toLowerCase()));
    ...
    /**
     * Same as {@link #tradeStream(String, WebSocketCallback)} plus accepts callbacks for all major websocket connection
     * events.
     *
     * @param symbol
     * @param onOpenCallback
     ...
     */
    @Override
    public int tradeStream(String symbol, WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
        WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback) {
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        Request request =
            RequestBuilder.buildWebsocketRequest(String.format("%s/ws/%s@trade", baseUrl, symbol.toLowerCase()));
        return createConnection(onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request);
    }
    ...
    /**
     * Same as {@link #klineStream(String, String, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param symbol
     * @param interval
    ...
    /**
     * Same as {@link #miniTickerStream(String, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param symbol
     * @param onOpenCallback
    ...
    public int miniTickerStream(String symbol, WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
        ...
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        Request request =
            RequestBuilder.buildWebsocketRequest(String.format("%s/ws/%s@miniTicker", baseUrl, symbol.toLowerCase()));
        return createConnection(onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request);
    }
    ...
    /**
     * Same as {@link #allMiniTickerStream(WebSocketCallback)} plus accepts callbacks for all major websocket connection
     * events.
     *
     * @param onOpenCallback
     * @param onMessageCallback
    ...
    /**
     * 24hr rolling window ticker statistics for a single symbol. These are NOT the statistics of the UTC day, but a
     * 24hr rolling window for the previous 24hrs. <br>
     * <br>
     * &lt;symbol&gt;@ticker <br>
     * <br>
     * Update Speed: Real-time
    ...
    /**
     * Same as {@link #symbolTicker(String, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param symbol
     * @param onOpenCallback
    ...
    public int symbolTicker(String symbol, WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
        ...
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        Request request =
            RequestBuilder.buildWebsocketRequest(String.format("%s/ws/%s@ticker", baseUrl, symbol.toLowerCase()));
        return createConnection(onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request);
    }

    /**
     * 24hr rolling window ticker statistics for all symbols that changed in an array. These are NOT the statistics of
     * the UTC day, but a 24hr rolling window for the previous 24hrs. Note that only tickers that have changed will be
    ...
    /**
     * Same as {@link #allTickerStream(WebSocketCallback)} plus accepts callbacks for all major websocket connection
     * events.
     *
     * @param onOpenCallback
     * @param onMessageCallback
    ...
    public int rollingWindowTicker(String symbol, String windowSize, WebSocketCallback callback) {
        ...
        return rollingWindowTicker(symbol.toLowerCase(), windowSize, noopCallback, callback, noopCallback,
            noopCallback);
    }

    /**
     * Same as {@link #rollingWindowTicker(String, String, WebSocketCallback)} (String, WebSocketCallback)} plus accepts
     ...
     */
    @Override
    public int rollingWindowTicker(String symbol, String windowSize, WebSocketCallback onOpenCallback,
        WebSocketCallback onMessageCallback, WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback) {
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        ParameterChecker.checkParameterType(symbol, String.class, "windowSize");
        ArrayList<String> allowedWindowSize = new ArrayList<String>() {
    ...
    /**
     * Rolling window ticker statistics for all market symbols, computed over multiple windows. Note that only tickers
     * that have changed will be present in the array. <br>
     * <br>
     * !ticker_&lt;window-size&gt;@arr <br>
     * <br>
     * Update Speed: Real-time
     *
     * @param windowSize
    ...
    /**
     * Same as {@link #allRollingWindowTicker(String, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param windowSize
     * @param onOpenCallback
     ...
     */
    @Override
    public int allRollingWindowTicker(String windowSize, WebSocketCallback onOpenCallback,
        WebSocketCallback onMessageCallback, WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback) {
        ParameterChecker.checkParameterType(windowSize, String.class, "windowSize");
        Request request = RequestBuilder
    ...
    /**
     * Same as {@link #bookTicker(String, WebSocketCallback)} plus accepts callbacks for all major websocket connection
     * events.
     *
     * @param symbol
     * @param onOpenCallback
    ...
    public int bookTicker(String symbol, WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
        ...
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        Request request =
            RequestBuilder.buildWebsocketRequest(String.format("%s/ws/%s@bookTicker", baseUrl, symbol.toLowerCase()));
        return createConnection(onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request);
    }
     ...
     * <br>
     * &lt;symbol&gt;@depth&lt;levels&gt;@&lt;speed&gt;ms <br>
     * <br>
    ...
    public int partialDepthStream(String symbol, int levels, int speed, WebSocketCallback callback) {
        ParameterChecker.checkParameterType(symbol, String.class, "symbol");
        return partialDepthStream(symbol.toLowerCase(), levels, speed, noopCallback, callback, noopCallback,
            noopCallback);
    }

    /**
     * Same as {@link #partialDepthStream(String, int, int, WebSocketCallback)} plus accepts callbacks for all major
    ...
    /**
     * Same as {@link #diffDepthStream(String, int, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param symbol
     * @param speed
    ...
    /**
     * Same as {@link #listenUserStream(String, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param listenKey
     * @param onOpenCallback
     ...
     *
     * @param streams
     *            A list of stream names to be combined <br>
     * @return int - Connection ID
    ...
    /**
     * Same as {@link #combineStreams(ArrayList, WebSocketCallback)} plus accepts callbacks for all major websocket
     * connection events.
     *
     * @param streams
     * @param onOpenCallback
    ...
    private int createConnection(WebSocketCallback onOpenCallback, WebSocketCallback onMessageCallback,
        WebSocketCallback onClosingCallback, WebSocketCallback onFailureCallback, Request request) {
        WebSocketConnection connection =
            new WebSocketConnection(onOpenCallback, onMessageCallback, onClosingCallback, onFailureCallback, request);
        connection.connect();
