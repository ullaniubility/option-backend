...
public class RocketConsumerSpringLifecycle implements SmartLifecycle {
    ...
    private void startConsumer(String name, DefaultMQPushConsumer consumer) {
        ...
        try {
            consumer.start();
            logger.info(String.format("启动RocketMQ Consumer监听器[%s]:%s，Topic=%s，Tag=%s - 成功", infos[1], infos[2],
                infos[3], infos[4]));
        } catch (MQClientException e) {
            logger.warn(String.format("启动RocketMQ Consumer监听器[%s]:%s，Topic=%s，Tag=%s - 失败", infos[1], infos[2],
                infos[3], infos[4]), e);
        }
    ...
    private void shutdownConsumer(String name, DefaultMQPushConsumer consumer) {
        ...
        String[] infos = name.split("-");
        logger.info(String.format("关闭RocketMQ Consumer监听器[%s]:%s，Topic=%s，Tag=%s - 成功", infos[1], infos[2], infos[3],
            infos[4]));
    }
